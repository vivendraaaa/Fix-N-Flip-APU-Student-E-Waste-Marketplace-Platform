...
$completed_repairs = $mysqli->query("SELECT COUNT(*) as count FROM repair_requests WHERE technician_id = $user_id AND status = 'completed'")->fetch_assoc()['count'] ?? 0;
$completed_disassembly = $mysqli->query("SELECT COUNT(*) as count FROM disassembly_requests WHERE technician_id = $user_id AND status = 'completed'")->fetch_assoc()['count'] ?? 0;
$approved_listings = $mysqli->query("SELECT COUNT(*) as count FROM technician_listings WHERE technician_id = $user_id AND status = 'approved'")->fetch_assoc()['count'] ?? 0;
?>
...
<div class="content-section">
    ...
    <div class="stats-grid" style="margin-bottom: 30px;">
        ...
        <div class="stat-card">
            <h3>Disassembled Devices</h3>
            <div class="value"><?php echo $completed_disassembly; ?></div>
        </div>
        <div class="stat-card">
            <h3>Approved Listings</h3>
            <div class="value"><?php echo $approved_listings; ?></div>
        </div>
    </div>
