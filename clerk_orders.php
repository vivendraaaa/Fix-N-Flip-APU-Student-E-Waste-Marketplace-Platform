...
<div class="content-section">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        ...
        <div>
            <select onchange="window.location.href='clerk.php?section=orders&status='+this.value" style="padding: 8px; border-radius: 6px; border: 1px solid #ddd;">
        ...
        <tbody>
            <?php while ($order = $orders->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><?php echo isset($order['order_date']) ? date('M d, Y H:i', strtotime($order['order_date'])) : 'N/A'; ?></td>
                    <td>
...
function viewOrder(id) {
    fetch('get_order_details.php?id=' + id)
        ...
        .then(data => {
            ...
            if (data.order_date) {
                html += '<div><strong>Date:</strong> ' + new Date(data.order_date).toLocaleString() + '</div>';
            }
...
function markOrderCompleted(orderId) {
    if (confirm('Mark this order as completed?')) {
        ...
        form.method = 'POST';
        form.innerHTML = '<input type="hidden" name="action" value="mark_completed"><input type="hidden" name="order_id" value="' + orderId + '">';
